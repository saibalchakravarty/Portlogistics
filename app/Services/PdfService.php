...
class PdfService {
    ...
    public function createPdf($challan, $planning, $organization, $origin, $destination, $auth) {
        ...
        if($shift_response['status']) {
            ...
            $response['result'] = $response['message'] = 'Error in getting shift details';
            return $response;
        }
        ...
        if($cargo_response['status']) {
            ...
            $response['status'] = false;
            $response['result'] = $response['message'] = 'Error in getting cargo details';
            return $response;
        }
        ...
        if($vessel_response['status']) {
            ...
            $response['status'] = false;
            $response['result'] = $response['message'] = 'Error in getting vessel details';
            return $response;
        }
        ...
        if($truck_response['status']) {
            ...
            $response['status'] = false;
            $response['result'] = $response['message'] = 'Error in getting truck details';
            return $response;
        }
        /**************Generate PDF*************/
        try {
            ...
            $mpdf->WriteHTML($html);
            $directory = 'org_'.$auth['user_auth']->organization_id . '/planning_'.$challan['btop_planning_id'].'_'.$challan['type'].'/pdf/';
            $pdf_path = storage_path('app/public/' . $directory);
            if (!File::exists($pdf_path)) {
                File::makeDirectory($pdf_path, $mode = 0755, true, true);
            }
            ...
            $response['status'] = false;
            $response['message'] = 'Unable to generate challan';
            return $response;
        }
